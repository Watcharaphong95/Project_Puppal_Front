...
class _ClinicmainPageState extends State<ClinicmainPage> {
                                                                          ...
                                                                          children: [
                                                                            TextSpan(
                                                                              ...
                                                                              style: TextStyle(
                                                                                ...
                                                                                color: Color(0xFF916B44).withOpacity(0.8),
                                                                              ),
                                                                            ),
                                                                            TextSpan(
                                                                              text: general.name ?? 'ไม่ระบุชื่อ',
                                                                              style: TextStyle(
                                                                      ...
                                                                      Expanded(
                                                                        child: (item.appointmentAid ==
                                                                            ...
                                                                            : FutureBuilder<List<dynamic>>(
                                                                                future: Future.wait([
                                                                                  getvaccine(item.appointmentAid.toString(), item.generalEmail),
                                                                                  getdog(
                                                                                    item.dogDogId is int ? item.dogDogId as int : int.tryParse(item.dogDogId.toString()) ?? 0,
                                                                                  ),
                                                                                ]),
                                                                                builder: (context, snapshot) {
                                                                                  ...
                                                                                  final appointmentData = snapshot.data?[0] as AppointmentClinic?;
                                                                                  final dogData = snapshot.data?[1] as DogDetailsPost?;

                                                                                  if (appointmentData == null || appointmentData.data.isEmpty) {
                                                                                    return Text(
                                                                                      'ไม่มีข้อมูลวัคซีน',
                                                                                      style: TextStyle(
                                                                                        fontSize: 13,
  ...
  Future<void> startRealtimeGet() async {
    ...
    try {
      final firstSnapshot = await FirebaseFirestore.instance
          .collection('reserve')
      ...
      if (_isDisposed) return;
      await _handleSnapshot(firstSnapshot);
      ...
      log("❌ Error in startRealtimeGet: $e");
    }
    ...
    try {
      ...
      for (var data in reserveList) {
        ...
        try {
          ...
          log("❌ Error getting reserve book for ${data.docId}: $e");
          continue;
        }
      }
  ...
  void dispose() {
    ...
    // ไม่มี delay แล้ว เพื่อไม่ให้ race condition
    super.dispose();
  }
  ...
  List<ReserveClinicFirebase> getEventsForDay(DateTime day) {
    ...
    return reservebookingListAll.where((item) {
      ...
      return eventDay == normalizedDay && item.status == 2;
    }).toList();
  }
